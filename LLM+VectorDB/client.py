...
client = genai.Client(api_key="")  # Unlike myself, use an environment variable or secure method to store your API key.
    ...
    async def initialize(self, user_query: str, server_path: str = "http://127.0.0.1:8050/sse") -> None:
        async with sse_client(server_path) as (self.read_stream, self.write_stream):
            async with ClientSession(self.read_stream, self.write_stream) as self.session:
                ...
                You have a tool called `fetch_context` that can be used to answer questions specific to research papers stored.

                this will search a vector database for relevant context to the paper and return the best match,so that you can answer
                 the user's question specifically and accurately.
...
async def main():
    mcpclient = MCPClient()
    await mcpclient.initialize(user_query="What department does the author 'Atharva Jayappa' of the paper belong to at Vishwakarma Institute of Technology?")  # Example query
